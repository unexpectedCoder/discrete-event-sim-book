...
class Environment:
    ...
    def process(self, proc, args=()):
        self.processes.append((proc, args))
    
    def timeout(self, delay, actions=[]):
    ...
    def run(self, until):
        for proc, args in self.processes:
            proc(*args)
...
def ball_motion(env: Environment, bounds, r, v, traj):
    ...
    env.timeout(
        ...
        actions=[
            ...
            (ball_motion, (env, bounds, r, v, traj))
        ]
    )
...
def update(env, dt, signs, traj, r, v):
    r += v * dt
    v *= signs
